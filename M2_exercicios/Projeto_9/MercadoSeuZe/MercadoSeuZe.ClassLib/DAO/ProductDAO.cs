...
namespace MercadoSeuZe.ClassLib
{
    public class ProductDAO
    {
        ...
        public void UpdateProductQuantityById(int quantity, string productId)
        {
            using (SqlConnection connection = new SqlConnection(_connectionString))
            {
                connection.Open();
                ...
                {
                    command.Connection = connection;
                    string sql = @"UPDATE Produtos SET quantidade_estoque = @quantidade_estoque WHERE produto_id = @produto_id;";

                    command.Parameters.AddWithValue("@produto_id", productId);
                    command.Parameters.AddWithValue("@quantidade_estoque", quantity);
                    command.CommandText = sql;
                    command.ExecuteNonQuery();
                }
            }
        ...
        public List<Product> SearchProductsByDescription(string description)
        {
            ...
            using (SqlConnection connection = new SqlConnection(_connectionString))
            {
                ...
                {
                    command.Connection = connection;
                    string sql = @"SELECT * FROM Produtos WHERE descricao LIKE '%' + LOWER(@descricao) +'%';";
                    command.CommandText = sql;

                    command.Parameters.AddWithValue("@descricao", description.ToLower());
        ...
        public Product SqlToObject(SqlDataReader reader)
        {
            ...
            newProduct.ProductId = Convert.ToInt64(reader["produto_id"]);
            newProduct.Name = reader["nome"].ToString();
            newProduct.Description = reader["descricao"].ToString();
            newProduct.ExpirationDate = Convert.ToDateTime(reader["data_vencimento"], CultureInfo.InvariantCulture);
